    ...
    <div className="min-h-screen bg-[#faf7f2] text-gray-900 font-sans">
      ...
      <nav className="bg-white/80 backdrop-blur-md shadow-sm sticky top-0 z-50 border-b border-gray-100">
        <div className="max-w-[1280px] mx-auto px-6 py-4 flex justify-between items-center">
          ...
          <div className="hidden md:flex gap-8 text-sm font-medium text-gray-600">
            <a href="#featured" className="hover:text-amber-600 transition">
              Featured
            </a>
            <a href="#trending" className="hover:text-amber-600 transition">
              Trending
            </a>
            <a href="#arrivals" className="hover:text-amber-600 transition">
              New Arrivals
            </a>
          ...
          <div className="hidden md:flex gap-4 items-center">
            <button className="text-gray-600 hover:text-amber-600 transition text-xl">
              🔍
            </button>
            <Link
              href="/cart"
              className="text-gray-600 hover:text-amber-600 transition text-xl relative"
            >
              🛍️
              {totalItems > 0 && (
            ...
            {user ? (
              <div className="flex gap-3 items-center">
                ...
                <button
                  onClick={logout}
                  className="bg-amber-600 text-white text-sm px-5 py-2 rounded-full hover:bg-amber-700 transition"
                >
                  Logout
                </button>
              </div>
        ...
        <div className="absolute inset-0 bg-gradient-to-r from-black/70 via-black/30 to-transparent flex items-center">
          <div className="max-w-[1280px] mx-auto px-6 md:px-12">
            ...
            <p className="mt-6 text-gray-300 text-lg max-w-md leading-relaxed">
              Premium menswear crafted for the modern man. Bold. Minimal.
              Timeless.
            </p>
            <div className="mt-8 flex gap-4">
              <a
                href="#featured"
                className="bg-amber-600 hover:bg-amber-700 text-white px-8 py-4 rounded-full font-medium transition"
              >
                Shop Now
              </a>
              <a
                href="#trending"
                className="border border-white text-white hover:bg-white hover:text-black px-8 py-4 rounded-full font-medium transition"
              >
                Explore
              </a>
            </div>
      ...
      <div className="bg-amber-600 text-white py-3 overflow-hidden whitespace-nowrap">
        <div className="animate-marquee inline-block">
          {Array(6)
            .fill(
              "FREE SHIPPING ON ORDERS ABOVE ৳5000  •  NEW COLLECTION 2026  •  PREMIUM QUALITY  •  EASY RETURNS  •  ",
            )
            .map((text, i) => (
              <span key={i} className="text-sm tracking-widest mx-8">
                {text}
              </span>
            ))}
        </div>
      ...
      <section className="max-w-[1280px] mx-auto px-6 py-20">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            <div
              key={cat.label}
              className={`${cat.bg} rounded-2xl p-8 flex flex-col items-center gap-3 cursor-pointer hover:shadow-md transition group`}
            >
      ...
      <div className="max-w-[1280px] mx-auto px-6">
        ...
        <section id="featured" className="py-16">
          <div className="flex justify-between items-end mb-12">
            ...
            <Link
              href="#"
              className="text-sm font-medium text-gray-500 hover:text-amber-600 transition border-b border-gray-300 pb-1"
            >
              View All →
            </Link>
          </div>
        ...
        <section className="my-16 rounded-3xl overflow-hidden relative h-80">
          ...
          <div className="absolute inset-0 bg-black/50 flex items-center justify-center text-center text-white">
            <div>
              <p className="text-amber-400 tracking-widest text-sm uppercase mb-3">
                Limited Edition
              </p>
        ...
        <div className="max-w-[1280px] mx-auto px-6 grid md:grid-cols-4 gap-10">
          <div>
            <h3 className="text-xl font-bold tracking-widest mb-4">
              MAN<span className="text-amber-500">VERSE</span>
            </h3>
            <p className="text-gray-400 text-sm leading-relaxed">
              Crafted for the modern man. Premium menswear for every occasion.
            </p>
          </div>
          <div>
          ...
          <div className="mb-4 w-80 bg-white border border-gray-200 rounded-2xl shadow-2xl flex flex-col overflow-hidden">
            <div className="bg-amber-600 px-4 py-3 flex items-center justify-between">
              ...
              <button
                ...
                className="text-white text-xl leading-none hover:text-amber-200 transition"
              >
                ×
              </button>
            </div>
            <div className="flex-1 p-4 space-y-3 overflow-y-auto max-h-80 bg-gray-50">
              {messages.length === 0 && (
                <div className="text-center">
                  <p className="text-gray-400 text-sm mt-4">
                    👋 Hi! I'm your Manverse assistant.
                  </p>
                  <div className="mt-4 flex flex-col gap-2">
                    {["What's new?", "Help with sizing", "Track my order"].map(
                      (q) => (
                        <button
                          key={q}
                          onClick={() => setInput(q)}
                          className="text-xs bg-white border border-gray-200 rounded-full px-4 py-2 text-gray-600 hover:border-amber-400 hover:text-amber-600 transition"
                        >
                          {q}
                        </button>
                      ),
              ...
              {messages.map((msg, i) => (
                ...
                >
                  <div
                    className={`px-4 py-2 rounded-2xl text-sm max-w-[85%] ${msg.role === "user" ? "bg-amber-600 text-white rounded-br-sm" : "bg-white text-gray-800 shadow-sm rounded-bl-sm"}`}
                  >
                    {msg.text}
                  </div>
                </div>
